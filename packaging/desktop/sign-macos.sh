...
# CodeRange", SIGTRAP). Re-sign the ONE binary that JITs with the
# committed entitlements plist (allow-jit + unsigned-executable-memory +
# disable-library-validation for native modules). The plist must stay
# COMMENT-FREE: AMFI rejects XML comments at load time while codesign
# still exits 0 — a broken signature that looks clean.
if [ -f "$ROOT/runtime/node" ]; then
  codesign --force --options runtime --timestamp \
    --entitlements "$PWD/packaging/desktop/node.entitlements.plist" \
    --keychain "$KEYCHAIN" --sign "$MACOS_SIGN_IDENTITY" "$ROOT/runtime/node"

  # PRE-1 gate: prove the SIGNED runtime still executes real JS before
  # pack + notarize. Notarization does NOT catch a missing JIT
  # entitlement, and `node --version` exits before V8 reserves its
  # code range — only real JS fails. Exactly how v0.1.0 shipped a
