...
| Increased-limit factors (occurrence/aggregate pairs) | If the filing prints one ILF per limit *pair*: a categorical `limit_pair` dimension + a 1-D table. If it prints a matrix (occurrence × aggregate): a 2-D table. Interpolated ILF curves: banded table at the filed breakpoints + `interpolation=linear` (registry-`partial`, split behavior per ADR-0063 — a 2-D matrix interpolates along its row axis; a 1-D banded curve rates stepped for now; the R-111 warning names which). |
