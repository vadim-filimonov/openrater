...
describe("parseZipTerritoryCsv", () => {
  it("seeds ZIP levels + a territory grouping from the KS map shape", () => {
    ...
    expect(r.territories.map((t) => t.id).sort()).toEqual(["701", "702"]);
    const t701 = r.territories.find((t) => t.id === "701")!;
    expect(t701.members).toEqual(["66101", "66102"]);
    expect(r.report.levelsCreated).toBe(4);
    expect(r.report.territories).toContainEqual({ id: "701", count: 2 });
    expect(r.report.territories).toContainEqual({ id: "702", count: 2 });
  });
  ...
  it("end-to-end with the canonical helpers — territory becomes the lookup key", async () => {
    ...
    expect(geoLookupKeys(dim).map((k) => k.id)).toEqual(["701", "702"]);
    expect(resolveGeographicValue(dim, "66101").key).toBe("701"); // ZIP → territory
    expect(resolveGeographicValue(dim, "702").key).toBe("702"); // idempotent
  });
  ...
  it("last row wins for a duplicate ZIP, moving it off the prior territory", () => {
    ...
    // 701 lost its only member → dropped; 66101 now under 702.
    expect(r.territories.map((t) => t.id)).toEqual(["702"]);
