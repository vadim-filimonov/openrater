 ...
 * exposure resolution.
 *
 * The runtime receives a `ClassLibrary` via `RunOptions.classLibrary` and passes
 ...
 * aggregate (with family, eligibility, NAICS, etc.) lives in the
 * application class registry; the review UI consumes a richer shape. The
 * runtime sees just this slice.
 *
 * Implementation note: the integrator wires their class registry to
 * this interface. For a synthetic in-memory seed library, a simple
 * Map-backed implementation suffices. For larger libraries served from a
...
export interface ClassLibraryEntry {
  /** The class code (e.g., "c201", "c101"). */
  readonly class_code: string;
  /** Display name shown in trace explanations (e.g., "Meridian
   *  Workshop", "Meridian Cafe"). */
 ...
 *
 * Useful for tests, conformance vectors, and a synthetic in-memory seed.
